...
int main(void)
{
	...
	t_art *tofind;
	tofind = dictSearch(dict, "INTERIOR - WHITE MOUNTAINS");
	if (tofind)
		printf("Found name=%s price=%d\n", tofind->name, tofind->price);
	else
		printf("Not found\n");

	tofind = dictSearch(dict, "Coloring #7");
	if (tofind)
		printf("Found name=%s price=%d\n", tofind->name, tofind->price);
	else
		printf("Not found\n");

	tofind = dictSearch(dict, "Games & Puzzles/Ball Puzzle");
	if (tofind)
		printf("Found name=%s price=%d\n", tofind->name, tofind->price);
	else
		printf("Not found\n");

	tofind = dictSearch(dict, "Gavno");
	if (tofind)
		printf("Found name=%s price=%d\n", tofind->name, tofind->price);
	else
		printf("Not found\n");
