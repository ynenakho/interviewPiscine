...
void printResult(t_node *node) {
	if (node)
		printf("%s\n", node->name);
	else
		printf("(404) Parent not found.\n");
}
...
int main(void)
{
	...
	node = findParent(root, "Homo sapiens", "Dinosauria");
	printResult(node);
	node = findParent(root, "Dinosauria", "Homo sapiens"); // returns the "Amniota" node
	printResult(node);
